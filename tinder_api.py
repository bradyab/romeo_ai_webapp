...
def get_read_receipts(match_id):
    ...
    '''
    try:
        r = requests.get('https://api.gotinder.com/readreceipt/' + match_id, headers=headers)
        print(r.text)
        print(r.status_code)
        return r.json()
    except requests.exceptions.RequestException as e:
...
def match_info_v2(match_id, count=60, message=0, page_token=None):
    try:
        url = host + '/v2/matches?locale=en&count=' + str(count) + '&message=' + str(message) + '&is_tinder_u=false/' + str(match_id)
        if page_token:
...
def all_matches(count=60, message=0, page_token=None):
    try:
        url = host + '/v2/matches?locale=en&count=' + str(count) + '&message=' + str(message) + '&is_tinder_u=false'
        if page_token:
            url = url + '&page_token=' + page_token
        r = requests.get(url, headers=headers)
        json = r.json()
