...
def submit():
	...
	try:
		...
		for name in ["profile", "updates"]:
			if name not in user:
				user[name] = []

		# only save data if it's been more than a day
		now = datetime.now(pytz.utc)
		if now - datetime.strptime(user["profile"][-1]["romeo_ai_saved_at"], '%Y-%m-%d %H:%M:%S.%f%z') > timedelta(days=1):
			...
			user["profile"][-1]["romeo_ai_saved_at"] = str(now)
			user["updates"] = [user["updates"]]
			user["updates"].append(user_updates)
			user["updates"][-1]["romeo_ai_saved_at"] = str(now)
